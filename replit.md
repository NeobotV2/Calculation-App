...
-   **Stundenverrechnungssatz-Kalkulator**: Professional hourly rate calculator at `#/stundensatz` with 5-step cost buildup: Basislohn (Tariflohn LG1 €15/h, Minijob/Teilzeit/Vollzeit) → SV AG-Anteil → Ausfallzeiten (Bundesland-based Feiertage) → Gemeinkosten → Gewinnmarge = Verrechnungssatz. Config stored in `hourlyRateConfig` (Zustand, store v4). Engine in `src/lib/hourly-rate-calc.ts`, Bundesland data in `src/data/bundeslaender.ts`.
