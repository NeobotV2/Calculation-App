...
-   **Interne Kalkulation (Druck)**: internal-only print view at `#/print/:id/intern` (entry: object options menu), clearly banner-marked "Nicht zur Weitergabe an Kunden" — LV table with effective Leistungswerte, per-hour cost chain (Basislohn → SV → Ausfall → Gemeinkosten → Vollkosten → Gewinn), Wirtschaftlichkeit (min/target price, margins, Spielraum) and risk factors. Customer offer print view at `#/print/:id` unchanged.
-   **Nachkalkulation**: per-object actual monthly hours (store slice `nachkalkulationen`, actions set/removeNachkalkulation, cleaned up on deleteProject/reset). Pure compare logic in `src/lib/nachkalkulation.ts` (hours deviation, real vs planned margin at fixed price, documented verdict thresholds ±2 %/+5 %; unit-tested). UI on the Controlling detail page (`#/auswertung/:id`): capture/update/remove Ist-Stunden with plan/actual comparison. The former "Profi-Controlling" card's hardcoded pseudo-percentages (Material 3 %, Lohn 70 %) were replaced with real Vollkosten/Gewinnbeitrag figures from the rate calculator.
-   **Wirtschaftlichkeits-Cockpit** (object detail): traffic-light economic status plus offer strategy derived transparently from the existing engines — Mindestpreis (rote Linie = hours × Vollkosten), Zielpreis (margin-on-price: rate = vollkosten/(1−m)), Verhandlungsspielraum, Deckungsbeitrag, Break-even-Satz, market verdict for €/m² — in `src/lib/price-strategy.ts`; sensitivity analysis (Lohn +5 %, Zeit +10 %, Preis −5 %; documented linearity assumption of Vollkosten in baseLohn) in the same module; risk scoring 0–100 with prioritized, actionable factors (margin, €/m² floor, missing Rüst-/Wegezeit buffers, optimistic performance overrides, default rate, staffing/FTE feasibility, incomplete LV) in `src/lib/risk-score.ts`. Benchmark bands live centrally in `src/data/benchmarks.ts` (documented orientation values, designed to be replaced by real Nachkalkulation/AI data later). UI: `pages/objekte/detail-parts/WirtschaftlichkeitPanel.tsx`. All three modules are unit-tested.
-   **Ausschreibungs-Kalkulation**: Public-tender tool at `#/ausschreibung` (entry points: Start page secondary CTA + "Mehr" hub). Imports a Leistungsverzeichnis from CSV (Excel export; auto-detects delimiter, header columns, German numbers, interval wording) or JSON (plain room arrays and the app's own export format) via `src/lib/lv-import.ts`, fuzzy-matches each position against the 125 room types (unmatched → default performance + warning), and computes a Bieterspanne via `src/lib/tender-calc.ts`: Mindestwert (perf +X %, rate −Y %), Mittelwert (base), Höchstwert (perf −X %, rate +Y %) with adjustable spreads (defaults ±15 %/±10 %). Rooms are editable via the shared RoomEditorSheet; the result can be converted into a regular Objekt (plan-gated). Both libs are unit-tested; analytics events `tender_lv_imported`/`tender_converted_to_object`.
