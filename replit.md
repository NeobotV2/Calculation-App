...
-   **Analytics**: Provider-agnostic analytics service (`src/services/analytics-service.ts`) with predefined event types for paywall views, upgrade triggers, subscription starts, onboarding, and free-limit events. Console logging in dev mode, pluggable provider via `setAnalyticsProvider()`.
