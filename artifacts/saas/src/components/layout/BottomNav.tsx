...
export function BottomNav() {
  ...
  return (
    <div
      className="fixed bottom-0 left-0 right-0 z-50 bg-background/98 border-t border-border/20 no-print"
    ...
    >
      <div className="flex items-center justify-around px-2 h-16 max-w-md mx-auto">
        {navItems.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} className="flex-1 flex flex-col items-center justify-center gap-1 h-full min-w-[44px] min-h-[44px]">
              <Icon size={20} strokeWidth={active ? 2.5 : 1.8} className={active ? "text-primary" : "text-muted-foreground"} />
